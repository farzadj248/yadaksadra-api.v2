...
class HomeController extends Controller
{
    public function adminHome(Request $request)
    {
        ...
        $products=Product::select('products.id','products.title','products.slug','products.main_inventory as inventory',
        'products.status', 'products_images.url as image','products.main_price as price','products.brand_name',
        'products.main_off as off','products.rating','products.views')
    ...
    public function webHome(Request $request)
    {
        $data = Cache::remember('web_home', 5, function () use($request){
            ...
            switch($role){
                case "Marketer":
                    $mare_sale_products->select('products.id','products.title','products.slug',"products.isFreeDelivery",
            ...
            switch($role){
                case "Marketer":
                    $amazin_products->select('products.id','products.title','products.slug',"products.isFreeDelivery",
                        'products.main_price_2','products.main_off_2','products.main_inventory_2',
                        'products.custom_price_2','products.custom_off_2','custom_inventory_2',
                        'products.market_price_2','products.market_off_2','products.market_inventory_2',
                    ...
                    ->where("user_type",5)
                    ->orWhere("user_type",1)
                    ->select("title","image_url","image_link")
                    ->get();
